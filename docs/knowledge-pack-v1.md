...
The same Study Analysis v1 proposal contract can be generated by a locally running Ollama model.
The provider accepts only numeric loopback traffic, submits a proposal-only JSON Schema to Ollama's
native non-streaming chat endpoint, and creates the authoritative bundle identity, analysis ID, and
`ollama:<model>` provenance locally. It then invokes the existing Study Analysis validator before
writing output. Unsupported model additions fail closed, and every valid proposal remains generated
until the existing named human-review gate approves it. See the
[local Ollama Study Provider contract](ollama-study-provider.md).
